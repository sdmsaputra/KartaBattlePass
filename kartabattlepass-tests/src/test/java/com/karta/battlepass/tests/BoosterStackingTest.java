...
public class BoosterStackingTest {
        ...
        List<ActiveBooster> boosters =
                List.of(
                        ...
                        createBooster(
                                "b1",
                                2.0,
                                BoosterStackingStrategy.SUM_DIFFERENT), // Should be ignored
    ...
    private ActiveBooster createBooster(
            String id, double multiplier, BoosterStackingStrategy strategy) {
        Booster booster =
                new Booster(
                        id,
                        "Test",
        ...
        return new ActiveBooster(
                booster, null, Instant.now(), Instant.now().plus(Duration.ofHours(1)));
    }
    ...
    private double calculateMultiplier(List<ActiveBooster> activeBoosters, BoosterType type) {
        ...
        Map<BoosterStackingStrategy, List<ActiveBooster>> byStrategy =
                activeBoosters.stream()
        ...
        List<ActiveBooster> sumDifferentGroup =
                byStrategy.get(BoosterStackingStrategy.SUM_DIFFERENT);
                                    ...
                                    group.stream()
                                            .max(
                                                    Comparator.comparingDouble(
                                                            b -> b.booster().multiplier()))
                                            .orElse(null)) // Find the best booster in each group
