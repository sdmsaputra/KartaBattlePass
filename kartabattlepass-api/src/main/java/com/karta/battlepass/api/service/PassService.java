...
public interface PassService {
     ...
     *     player's {@link Pass} if they have one, or an empty Optional otherwise.
     */
    @NotNull
     ...
     *
     * @param playerUuid The UUID of the player.
     * @return A {@link CompletableFuture} that completes with {@code true} if the player has a
     *     premium pass, {@code false} otherwise.
     */
    @NotNull
    CompletableFuture<Boolean> hasPremiumPass(@NotNull UUID playerUuid);

    /**
     * Gives a player a pass of a specific type for the current season. If the player already has a
     * pass of the same or higher type, this operation may do nothing. For example, giving a FREE
     * pass to a player who already has a PREMIUM pass will not downgrade them.
     *
     * @param playerUuid The UUID of the player to give the pass to.
     * @param type The type of pass to give.
     * @return A {@link CompletableFuture} that completes with the resulting {@link Pass} for the
     *     player.
     */
    @NotNull
