...
public interface PlayerProgressDao {
    ...
    @SqlUpdate(
            "INSERT INTO kbp_player_progress (player_uuid, points, tier, updated_at) "
    ...
    @SqlUpdate(
            "UPDATE kbp_player_progress SET points = :points, tier = :tier, updated_at = :now "
    ...
    @SqlUpdate(
            "UPDATE kbp_player_progress SET points = points + :amount, updated_at = :now "
                    + "WHERE player_uuid = :uuid")
