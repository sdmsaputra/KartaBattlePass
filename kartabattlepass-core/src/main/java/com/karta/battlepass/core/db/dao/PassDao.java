...
public interface PassDao {
    ...
    Optional<PassRecord> findByPlayerAndSeason(@Bind("uuid") UUID uuid, @Bind("seasonId") long seasonId);

    @SqlUpdate("INSERT INTO kbp_passes (season_id, player_uuid, pass_type, purchase_time) " +
               "VALUES (:seasonId, :uuid, :type, :now)")
    @GetGeneratedKeys
    long insert(@Bind("seasonId") long seasonId, @Bind("uuid") UUID uuid, @Bind("type") String type, @Bind("now") Instant now);
}
