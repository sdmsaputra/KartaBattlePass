...
public interface QuestProgressDao {
    ...
    Optional<QuestProgressRecord> findById(@Bind("uuid") UUID uuid, @Bind("questId") String questId);
    ...
    @SqlUpdate("INSERT INTO kbp_quest_progress (player_uuid, quest_id, progress_data, status, updated_at) " +
               "VALUES (:playerUuid, :questId, :progressData, :status, :updatedAt)")
    void insert(@BindBean QuestProgressRecord record);

    @SqlUpdate("UPDATE kbp_quest_progress SET progress_data = :progressData, status = :status, updated_at = :updatedAt " +
