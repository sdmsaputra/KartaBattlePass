...
public class PassServiceImpl implements PassService {
    ...
    public @NotNull CompletableFuture<Boolean> hasPremiumPass(@NotNull UUID playerUuid) {
        return getPass(playerUuid)
                .thenApply(
                        passOpt ->
                                passOpt.map(pass -> pass.type() == PassType.PREMIUM).orElse(false));
    }

    @Override
    public @NotNull CompletableFuture<Pass> givePass(
            @NotNull UUID playerUuid, @NotNull PassType type) {
        return registry.seasonService()
                .getCurrentSeason()
                .thenCompose(
                        seasonOpt -> {
                            if (seasonOpt.isEmpty()) {
                                throw new IllegalStateException(
                                        "No active season to give a pass for.");
                            }
                                    ...
                                    () ->
                                            jdbi.inTransaction(
                                                    handle -> {
                                                        ...
                                                        Optional<PassRecord> existing =
                                                                dao.findByPlayerAndSeason(
                                                                        playerUuid, seasonId);

                                                        if (existing.isPresent()) {
                                                            // Don't downgrade a premium pass
                                                            if (existing.get().passType()
                                                                            == PassType.PREMIUM
                                                                    || type == PassType.FREE) {
                                                                return toApi(existing.get());
                                                            }
                                                            // Upgrade to premium is not handled
                                                            // here, should be a separate method
                                                            // This method is for granting, not
                                                        ...
                                                        long newId =
                                                                dao.insert(
                                                                        ...
                                                                        type.name(),
                                                                        Instant.now());
                                                        // Invalidate player profile cache so it
                                                        // picks up the new pass type on next load
                                                        registry.playerService()
    ...
    private Pass toApi(PassRecord record) {
        return new Pass(
                record.playerUuid(), record.seasonId(), record.passType(), record.purchaseTime());
    }
}
