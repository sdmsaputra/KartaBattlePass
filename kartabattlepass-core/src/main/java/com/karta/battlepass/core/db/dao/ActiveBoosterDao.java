...
public interface ActiveBoosterDao {
    ...
    @SqlUpdate(
            "INSERT INTO kbp_active_boosters (booster_def_id, player_uuid, activated_at, expires_at) "
                    + "VALUES (:boosterDefId, :playerUuid, :activatedAt, :expiresAt)")
    @GetGeneratedKeys
    long insert(@BindBean ActiveBoosterRecord record);

    @SqlQuery(
            "SELECT * FROM kbp_active_boosters WHERE expires_at > :now AND (player_uuid = :uuid OR player_uuid IS NULL)")
    List<ActiveBoosterRecord> findActiveForPlayer(
            @Bind("uuid") UUID uuid, @Bind("now") Instant now);
