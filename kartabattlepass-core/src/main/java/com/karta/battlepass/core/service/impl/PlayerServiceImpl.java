...
public class PlayerServiceImpl implements PlayerService {
    ...
    public PlayerServiceImpl(
            ...
            @NotNull Jdbi jdbi,
            @NotNull KartaScheduler scheduler,
        ...
        this.profileCache =
                Caffeine.newBuilder()
                        ...
                        .buildAsync(
                                (uuid, executor) ->
                                        loadPlayerProfile(uuid, "Unknown")); // Name is unknown here
    }
                        ...
                        jdbi.inTransaction(
                                handle -> {
                                    ...
                                    PlayerProgressDao progressDao =
                                            handle.attach(PlayerProgressDao.class);

                                    Instant now = Instant.now();
                                    Optional<PlayerRecord> playerRecordOpt =
                                            playerDao.findById(uuid);
                                    ...
                                    PlayerProgressRecord progressRecord =
                                            progressDao
                                                    .findById(uuid)
                                                    .orElse(
                                                            new PlayerProgressRecord(
                                                                    uuid, 0, 0, now));
                                    ...
                                    PassType passType =
                                            registry.passService()
                                                    .getPass(uuid)
                                                    .join() // This is safe as we are inside a
                                                    // supplyAsync chain
                                                    .map(Pass::type)
    ...
    /**
     * Handles the logic for when a player joins the server. Ensures the player is registered in the
     * database.
     *
    ...
    public CompletableFuture<Void> handlePlayerJoin(UUID uuid, String name) {
        // Preload the cache
        return profileCache
                .get(uuid, (u, exe) -> loadPlayerProfile(u, name))
                .thenAccept(profile -> {});
    }

    @Override
    public @NotNull CompletableFuture<Optional<PlayerProfile>> getPlayerProfile(
                    ...
                    jdbi.useHandle(
                            handle -> {
                                ...
                                dao.update(
                                        ...
                                        amount,
                                        0,
                                        Instant.now()); // Tier calculation needed
                            });
                    profileCache.synchronous().invalidate(playerUuid);
                    return null;
                });
    }
