...
import com.karta.battlepass.core.db.dao.QuestProgressDao;
...
public class QuestServiceImpl implements QuestService {
    ...
    @Override
    public @NotNull CompletableFuture<List<Quest>> getAvailableQuests(@NotNull UUID playerUuid, @NotNull QuestCategory category) {
    ...
    @Override
    public @NotNull CompletableFuture<QuestProgress> getQuestProgress(@NotNull UUID playerUuid, @NotNull String questId) {
        // TODO: Implement DAO call
        return null;
    }

    @Override
    public @NotNull CompletableFuture<Void> forceCompleteQuest(@NotNull UUID playerUuid, @NotNull String questId) {
        // TODO: Implement logic
        return CompletableFuture.completedFuture(null);
    }

    @Override
    public @NotNull CompletableFuture<Void> resetQuestProgress(@NotNull UUID playerUuid, @NotNull String questId) {
        // TODO: Implement logic
        return CompletableFuture.completedFuture(null);
    }
    ...
    public @NotNull CompletableFuture<Void> reloadQuests() {
        return CompletableFuture.runAsync(() -> {
            questRegistry.clear();
            Map<String, QuestConfig> configs = configManager.loadConfigsFromDirectory("quests", QuestConfig.class);
            for (QuestConfig config : configs.values()) {
                config.quests().forEach((id, def) -> {
    ...
    public void processQuestProgress(@NotNull UUID playerUuid, @NotNull Object event) {
        ...
        getAvailableQuests(playerUuid, null).thenAccept(quests -> {
            for (Quest quest : quests) {
                QuestType questType = questTypeRegistry.get(quest.type().toLowerCase());
                if (questType != null && questType.getListeningEvent().isInstance(event)) {
                    // This part needs a proper implementation with the QuestContext
                    // and database updates. It's a placeholder for now.
                    System.out.println("Processing quest " + quest.id() + " for event " + event.getClass().getSimpleName());
                }
            }
        });
    }
    ...
    @Override
    public @NotNull CompletableFuture<Void> incrementQuestProgress(@NotNull UUID playerUuid, @NotNull String questId, int amount) {
    ...
    private Quest toApi(String id, QuestConfig.QuestDefinition def) {
        List<Reward> rewards = def.rewards().stream()
                .map(r -> new Reward(
                        id + "_" + r.type(), // Generate a unique ID for the reward
                        r.type(),
                        r.data(),
                        r.description()
                )).collect(Collectors.toList());
