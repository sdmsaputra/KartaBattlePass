...
public interface RewardClaimedDao {
    ...
    @SqlUpdate("INSERT INTO kbp_rewards_claimed (player_uuid, reward_id, claimed_at) VALUES (:playerUuid, :rewardId, :claimedAt)")
    ...
    @SqlQuery("SELECT * FROM kbp_rewards_claimed WHERE player_uuid = :uuid AND reward_id = :rewardId")
    Optional<RewardClaimedRecord> findById(@Bind("uuid") UUID uuid, @Bind("rewardId") String rewardId);
}
