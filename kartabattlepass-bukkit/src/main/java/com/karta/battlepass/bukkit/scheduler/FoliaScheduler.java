...
public class FoliaScheduler implements KartaScheduler {
    ...
    public @NotNull KartaTask runLaterAsync(@NotNull Runnable task, @NotNull Duration delay) {
        return toKartaTask(plugin.getServer().getAsyncScheduler().runDelayed(plugin, t -> task.run(), delay.toMillis(), TimeUnit.MILLISECONDS));
    }

    @Override
    public @NotNull KartaTask runLaterSync(@NotNull Runnable task, @NotNull Duration delay) {
        return toKartaTask(plugin.getServer().getGlobalRegionScheduler().runDelayed(plugin, t -> task.run(), delay.toSeconds() * 20));
    }

    @Override
    public @NotNull KartaTask runTimerAsync(@NotNull Runnable task, @NotNull Duration delay, @NotNull Duration period) {
        return toKartaTask(plugin.getServer().getAsyncScheduler().runAtFixedRate(plugin, t -> task.run(), delay.toMillis(), period.toMillis(), TimeUnit.MILLISECONDS));
    }

    @Override
    public @NotNull KartaTask runTimerSync(@NotNull Runnable task, @NotNull Duration delay, @NotNull Duration period) {
        return toKartaTask(plugin.getServer().getGlobalRegionScheduler().runAtFixedRate(plugin, t -> task.run(), delay.toSeconds() * 20, period.toSeconds() * 20));
    }
